...
class TestEVPNManager:
    ...
    async def test_process_device_connection_failure(self, manager):
        ...
        with patch.object(EVPNStatusChecker, 'connect', new_callable=AsyncMock) as mock_connect:
    ...
    async def test_process_device_success_no_rejected(self, manager):
        ...
        with patch.object(EVPNStatusChecker, 'connect', new_callable=AsyncMock) as mock_connect, \
             patch.object(EVPNStatusChecker, 'get_evpn_route_status') as mock_status_method, \
             patch.object(EVPNStatusChecker, 'disconnect') as mock_disconnect:
    ...
    async def test_run_with_devices(self, manager, capsys):
        ...
        mock_results = [
            {
                'host': '192.168.1.100',
                'connected': True,
                'status_counts': {'Accepted': 10, 'Rejected': 0, 'Pending': 0, 'Invalid': 0, 'Unknown': 0},
                'restart_attempted': False,
                'restart_success': False
            },
            {
                'host': '192.168.1.101',
                'connected': True,
                'status_counts': {'Accepted': 5, 'Rejected': 2, 'Pending': 1, 'Invalid': 0, 'Unknown': 0},
                'restart_attempted': False,
                'restart_success': False
            }
        ]
        ...
        with patch.object(manager, 'load_hosts') as mock_load, \
             patch.object(manager, 'process_device', new_callable=AsyncMock) as mock_process:
        ...
        mock_results = [
            {
                ...
                'name': '192.168.1.100',
                'connected': True,
                'status_counts': {'Accepted': 5, 'Rejected': 2, 'Pending': 0, 'Invalid': 0, 'Unknown': 0},
                'restart_attempted': True,
                'restart_success': True
            }
        ]
        
        with patch.object(fix_manager, 'load_hosts') as mock_load, \
             patch.object(fix_manager, 'process_device', new_callable=AsyncMock) as mock_process:
            ...
            assert "Routing restart: SUCCESS" in captured.out
